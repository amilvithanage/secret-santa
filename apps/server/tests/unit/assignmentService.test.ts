...
describe('AssignmentService', () => {
  ...
  describe('createAssignments', () => {
    ...
    it('should throw NotFoundError when gift exchange does not exist', async () => {
      ...
      await expect(assignmentService.createAssignments(request)).rejects.toThrow(
        NotFoundError
      );
      await expect(assignmentService.createAssignments(request)).rejects.toThrow(
    ...
    it('should throw ValidationError when not enough participants', async () => {
      ...
      mockPrisma.giftExchange.findUnique.mockResolvedValue(mockGiftExchangeWithOneParticipant);
    ...
    it('should throw ValidationError when assignments already exist', async () => {
      ...
      mockPrisma.giftExchange.findUnique.mockResolvedValue(mockGiftExchangeWithAssignments);
  ...
  describe('getAssignmentById', () => {
    ...
    it('should return null when assignment not found', async () => {
      ...
      const result = await assignmentService.getAssignmentById('non-existent-id');
  ...
  describe('getAssignmentsByGiftExchange', () => {
    it('should return assignments for a gift exchange', async () => {
      ...
      const result = await assignmentService.getAssignmentsByGiftExchange('exchange-1');
  ...
  describe('Secret Santa Algorithm Edge Cases', () => {
    it('should handle assignments with exclusion rules', async () => {
      ...
      mockPrisma.giftExchange.findUnique.mockResolvedValue(mockGiftExchangeWithExclusions);
      mockPrisma.$transaction.mockImplementation(async (callback: any) => {
        return await callback(mockTransaction);
      });
      mockTransaction.assignment.create.mockResolvedValue(mockCreatedAssignments[0]);
    ...
    it('should return error when no valid assignments possible', async () => {
      ...
      mockPrisma.giftExchange.findUnique.mockResolvedValue(mockGiftExchangeImpossible);
    ...
    it('should handle database errors during assignment creation', async () => {
      ...
      const mockGiftExchange = {
        ...
        participants: [
          {
            id: 'gep-1',
            participant: { id: 'participant-1', name: 'John', email: 'john@example.com' },
          },
          {
            id: 'gep-2',
            participant: { id: 'participant-2', name: 'Jane', email: 'jane@example.com' },
          },
        ],
        assignments: [],
        exclusionRules: [],
