...
describe("GiftExchangeService", () => {
  ...
  beforeEach(() => {
    ...
    (
      ParticipantService as jest.MockedClass<typeof ParticipantService>
  ...
  describe("createGiftExchange", () => {
    it("should create a new gift exchange successfully", async () => {
      const request: CreateGiftExchangeRequest =
        createMockGiftExchangeRequest();
    ...
    it("should throw ConflictError when duplicate name and year exists", async () => {
      ...
      await expect(
        giftExchangeService.createGiftExchange(request),
    ...
    it("should throw ValidationError for empty name", async () => {
      ...
      jest
        .spyOn(giftExchangeService, "createGiftExchange")
        .mockRejectedValue(new ValidationError("Name is required"));
    ...
    it("should throw ConflictError for duplicate name and year", async () => {
      ...
      jest
        ...
        .mockRejectedValue(
          new BadRequestError(
            "Gift exchange with this name and year already exists",
          ),
        );
  ...
  describe("getGiftExchangeById", () => {
    it("should return gift exchange by id", async () => {
      ...
      const result =
        await giftExchangeService.getGiftExchangeById("exchange_123");
    ...
    it("should throw NotFoundError when exchange does not exist", async () => {
      ...
      await expect(
        giftExchangeService.getGiftExchangeById("nonexistent"),
  ...
  describe("updateGiftExchange", () => {
    it("should update gift exchange successfully", async () => {
      ...
      mockPrisma.giftExchange.findUnique.mockResolvedValue(
        createMockPrismaGiftExchange(),
      );
      // Mock duplicate check
      mockPrisma.giftExchange.findFirst.mockResolvedValue(null);
      mockPrisma.giftExchange.update.mockResolvedValue(
        mockUpdatedPrismaExchange,
      );
  ...
  describe("deleteGiftExchange", () => {
    it("should delete gift exchange successfully", async () => {
      ...
      await expect(
        giftExchangeService.deleteGiftExchange("exchange_123"),
  ...
  describe("addParticipantToExchange", () => {
    ...
    it("should throw NotFoundError when participant does not exist", async () => {
      ...
      mockParticipantService.getParticipantById.mockRejectedValue(
        new NotFoundError("Participant not found"),
      );
  ...
  describe("removeParticipantFromExchange", () => {
    it("should remove participant from gift exchange successfully", async () => {
      ...
      const result = await giftExchangeService.removeParticipantFromExchange(
        "exchange_123",
        "participant_123",
      );
    ...
    it("should throw NotFoundError when participant not in exchange", async () => {
      ...
      await expect(
        giftExchangeService.removeParticipantFromExchange(
          "exchange_123",
  ...
  describe("getExchangeParticipants", () => {
    it("should return all participants in gift exchange", async () => {
      ...
      mockPrisma.giftExchangeParticipant.findMany.mockResolvedValue([
        {
          participant: createMockParticipant({ id: "participant_1" }),
          createdAt: new Date(),
        },
        {
          participant: createMockParticipant({ id: "participant_2" }),
          createdAt: new Date(),
        },
      ]);

      const result =
        await giftExchangeService.getExchangeParticipants("exchange_123");
...
function createMockGiftExchangeRequest(
  overrides?: Partial<CreateGiftExchangeRequest>,
): CreateGiftExchangeRequest {
  return {
    name: "Christmas 2024",
    year: 2024,
