...
export class GiftExchangeService {
  ...
  async createGiftExchange(data: CreateGiftExchangeRequest): Promise<GiftExchangeResponse> {
    ...
    if (existingExchange) {
      throw new ConflictError(`A gift exchange with name "${data.name}" already exists for year ${data.year}`);
    }
    ...
    try {
      ...
      if (error.code === 'P2002') {
        throw new ConflictError('A gift exchange with this name and year already exists');
      }
      throw error;
    }
  }
   ...
   */
  async getGiftExchanges(page: number = 1, limit: number = 10): Promise<GiftExchangeResponse[]> {
  ...
  async updateGiftExchange(id: string, data: UpdateGiftExchangeRequest): Promise<GiftExchangeResponse> {
    ...
    if (data.status) {
      this.validateStatusTransition(existingExchange.status as GiftExchangeStatus, data.status);
    }
  ...
  async deleteGiftExchange(id: string): Promise<void> {
    ...
    if (giftExchange.status === GiftExchangeStatus.ASSIGNED || giftExchange.status === GiftExchangeStatus.COMPLETED) {
      throw new ValidationError('Cannot delete gift exchange with assignments. Please reset assignments first.');
    }
   ...
   */
  async addParticipantToExchange(giftExchangeId: string, data: AddParticipantToExchangeRequest): Promise<void> {
    ...
    if (existingParticipant) {
      throw new ConflictError('Participant is already part of this gift exchange');
    }

    // Prevent adding participants if assignments have been made
    if (giftExchange.status === GiftExchangeStatus.ASSIGNED || giftExchange.status === GiftExchangeStatus.COMPLETED) {
      throw new ValidationError('Cannot add participants to gift exchange with assignments');
    }
    ...
    if (giftExchange.status === GiftExchangeStatus.DRAFT) {
      await this.updateGiftExchange(giftExchangeId, { status: GiftExchangeStatus.PARTICIPANTS_ADDED });
    }
  }
   ...
   */
  async removeParticipantFromExchange(giftExchangeId: string, participantId: string): Promise<GiftExchangeResponse> {
    // Validate gift exchange exists
    const giftExchange = await this.getGiftExchangeById(giftExchangeId);

    // Prevent removing participants if assignments have been made
    if (giftExchange.status === GiftExchangeStatus.ASSIGNED || giftExchange.status === GiftExchangeStatus.COMPLETED) {
      throw new ValidationError('Cannot remove participants from gift exchange with assignments');
    }
   ...
   */
  async validateParticipantsInExchange(giftExchangeId: string, participantIds: string[]): Promise<void> {
    const participants = await this.db.giftExchangeParticipant.findMany({
      where: {
        giftExchangeId,
    ...
    if (participants.length !== participantIds.length) {
      ...
      throw new ValidationError(`Participants not found in exchange: ${missingIds.join(', ')}`);
    }
  }

  private validateStatusTransition(currentStatus: GiftExchangeStatus, newStatus: GiftExchangeStatus): void {
    const validTransitions: Record<GiftExchangeStatus, GiftExchangeStatus[]> = {
      ...
      [GiftExchangeStatus.PARTICIPANTS_ADDED]: [GiftExchangeStatus.ASSIGNED, GiftExchangeStatus.DRAFT],
      [GiftExchangeStatus.ASSIGNED]: [GiftExchangeStatus.COMPLETED, GiftExchangeStatus.PARTICIPANTS_ADDED],
