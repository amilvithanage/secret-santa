...
import { CreateAssignmentsRequest, AssignmentResponse, AssignmentResult } from '@secret-santa/shared-types';
...
export class AssignmentService {
   ...
   */
  async createAssignments(data: CreateAssignmentsRequest): Promise<AssignmentResult> {
    try {
      ...
      if (giftExchange.participants.length < 2) {
        throw new ValidationError('Gift exchange must have at least 2 participants');
      }

      // Check if assignments already exist
      if (giftExchange.assignments.length > 0) {
        throw new ValidationError('Assignments already exist for this gift exchange');
      }

      // Generate assignments using Secret Santa algorithm
      const assignmentPairs = await this.generateSecretSantaAssignments(
        giftExchange.participants.map((p: { participant: any }) => p.participant),
        giftExchange.exclusionRules
      );

      if (!assignmentPairs) {
        throw new ValidationError('Unable to generate valid assignments with current exclusion rules');
      }
   ...
   */
  async getAssignmentsByGiftExchange(giftExchangeId: string): Promise<AssignmentResponse[]> {
    const assignments = await this.db.assignment.findMany({
  ...
  private async generateSecretSantaAssignments(
    ...
    if (this.findAssignment(participantIds, 0, exclusions, assignments, usedReceivers)) {
      return assignments;
    }

    // If backtracking fails, try multiple random shuffles
    for (let attempt = 0; attempt < 100; attempt++) {
      const shuffledGivers = [...participantIds].sort(() => Math.random() - 0.5);
      const shuffledReceivers = [...participantIds].sort(() => Math.random() - 0.5);

      const tempAssignments: { giverId: string; receiverId: string }[] = [];
      const tempUsedReceivers = new Set<string>();

      if (this.findAssignment(shuffledGivers, 0, exclusions, tempAssignments, tempUsedReceivers)) {
  ...
  private findAssignment(
    ...
    for (const receiverId of givers) {
      if (!usedReceivers.has(receiverId) && !excludedReceivers.has(receiverId)) {
        // Try this assignment
        assignments.push({ giverId, receiverId });
        usedReceivers.add(receiverId);

        // Recursively assign remaining givers
        if (this.findAssignment(givers, giverIndex + 1, exclusions, assignments, usedReceivers)) {
          return true;
        }
